package uz.ciasev.ubdd_service.service.webhook.sit_center.dto;
...
public class SitCenterWebhookEventProtocolDecisionDataDTO {
    ...
    private final String violatorGender;
    @JsonFormat(pattern = "dd-MM-yyyy 00:00:00")
    private final LocalDate violatorBirthdate;

    private final Long violatorRegionId;
    private final Long violatorDistrictId;
    private final Long intoxicationTypeId;
    private final String intoxicationTypeName;
    ...
    @JsonFormat(pattern = "dd-MM-yyyy 00:00:00")
    private final LocalDate inspectorBirthdate;
    ...
    public SitCenterWebhookEventProtocolDecisionDataDTO(SitCenterWebhookProtocolDecisionProjection p) {
        ...
        this.violationTime = p.getViolationTime();
        this.violatorGender = p.getViolatorGender();
        this.violatorBirthdate = p.getBirthDate();
        this.inspectorFio = p.getInspectorFio();
        this.inspectorBirthdate = p.getInspectorBirthdate();
        this.inspectorWorkCertificate = p.getInspectorWorkCertificate();

        this.intoxicationTypeId = p.getViolatorIntoxicationId();
        this.intoxicationTypeName = p.getViolatorIntoxicationName();
        this.violatorRegionId = p.getViolatorActualAddressRegionId();
        this.violatorDistrictId = p.getViolatorActualAddressDistrictId();
    }
}
