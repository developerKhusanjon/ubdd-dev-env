...
import uz.ciasev.ubdd_service.entity.webhook.sit_center.SitCenterWebhookProtocolDecisionProjection;

import java.util.List;

public interface SitCenterGenerateWebhookEventRepository extends JpaRepository<SitCenterGenerateWebhookEvent, Long> {
    ...
    @Query("SELECT " +
            ...
            "jsonb_extract_path_text(violatorGender.name, 'lat') AS violatorGender, " +
            "jsonb_extract_path_text(intoxicationType.name, 'lat') AS violatorIntoxicationName, " +
            "violatorDetail.intoxicationTypeId AS violatorIntoxicationId, " +
            "violatorActualAddress.regionId AS violatorActualAddressRegionId, " +
            "violatorActualAddress.districtId AS violatorActualAddressDistrictId, " +
            ...
            "LEFT JOIN IntoxicationType AS intoxicationType ON intoxicationType.id=violatorDetail.intoxicationTypeId " +

            "LEFT JOIN Address AS violatorActualAddress ON violatorActualAddress.id=violator.actualAddressId " +
...
//            Нужно добавить эти таблицы n
    ...
    List<SitCenterWebhookProtocolDecisionProjection> getProjectionsByProtocol(@Param("id")Long protocoId);
}